#include "cmake-build-debug/interface.h"
#include "cmake-build-debug/MyImplementation.h"
...
void test4(Ex2* ex2)
{
    ...
    file.close();
    Flight* flightcheck = ex2->getFlight(ids[0]);
    string checkflightid = flightcheck->getID();
    Flight* flight = (Flight*) should_succeed(ex2, [ids](Ex2* ex2) {return (void*)ex2->getFlight(ids[0]); }, counter++);
    vector<MyPlane> checkplane =  DataBase::planeTable;
    vector<MyEmployee> checkemp = DataBase::employeeTable;
    vector<MyReservation> checkres = DataBase::reservationTable;
    vector<MyCustomer> checkcus = DataBase::customerTable;
    vector<MyFlight> checkf = DataBase::flightTable;
    vector<takenEmployees> checkte = DataBase::takenEmployeesTable;
